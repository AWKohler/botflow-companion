...
Anisette data is obtained via Anisette.py (pure-Python, Unicorn emulation)
or from an HTTP anisette server (macOS uses native AOSKit).
Credentials are NEVER stored to disk — only held in memory for the session.

Dependencies: srp, cryptography, requests, anisette (Linux)
"""
...
class AnisetteProvider:
    """
    HTTP-based anisette provider — fetches data from an external server.

    Used as fallback on Windows or when --anisette-url is provided.
    On Linux, prefer LinuxAnisetteProvider (pure-Python, no server needed).
    On macOS, prefer NativeAnisetteProvider (uses AOSKit.framework).
    """
    ...
    def fetch(self) -> dict:
        """Fetch fresh anisette data from HTTP anisette server."""
        try:
            ...
            raise ConnectionError(
                ...
                f"On Linux, install the 'anisette' package (pip install anisette) — no server needed.\n"
                f"Or start an external anisette HTTP server on {self.server_url}"
            )
    ...
    def generate_headers(self) -> dict:
        ...
        # Use server-provided identifiers — OTP data (MD, MD-M) is
        # cryptographically bound to these; mismatches cause ec=-22421.
        device_id = anisette.get("X-Mme-Device-Id", self.device_id)
        md_lu = anisette.get("X-Apple-I-MD-LU",
                             base64.b64encode(self.local_user_id.encode()).decode())
        # Client-Info must match the anisette device identity
        client_info = (anisette.get("X-Mme-Client-Info")
                       or anisette.get("X-MMe-Client-Info")
                       or CLIENT_INFO)
...
# ─── Linux Anisette Provider (pure Python, no Docker) ────────────────────────

class LinuxAnisetteProvider(AnisetteProvider):
    """
    Pure-Python anisette provider using Anisette.py (malmeloo/Anisette.py).

    Uses Unicorn Engine to emulate Apple's ARM64 ADI libraries entirely in
    userspace — no Docker, no native binaries, no OpenSSL dependency.

    First run downloads ~3 MB of Apple libs (cached automatically).
    Session data is persisted in ~/.config/cc-ios/anisette/session.bin.

    Install: pip install anisette
    """

    _SESSION_DIR = Path.home() / ".config" / "cc-ios" / "anisette"
    _SESSION_FILE = _SESSION_DIR / "session.bin"
    ...
    def __init__(self):
        try:
            ...
            raise ImportError(
                "Anisette library not installed. Run: pip install anisette"
            )
        ...
        if self._SESSION_FILE.exists():
            try:
                self._ani = Anisette.load(str(self._SESSION_FILE))
            except Exception:
                print("[*] Anisette session corrupted, re-initializing...")
                self._SESSION_FILE.unlink(missing_ok=True)
                self._ani = Anisette.init()
        else:
            print("[*] Initializing Anisette.py (first run downloads libs ~3 MB)...")
            self._ani = Anisette.init()

        self._save()

        # Set stable identifiers from the session
        data = self._ani.get_data()
        self.device_id = data.get("X-Mme-Device-Id", str(uuid.uuid4()).upper())
        self.local_user_id = data.get(
            "X-Apple-I-MD-LU", str(uuid.uuid4()).upper()
        )
        print("[+] Anisette: Python native (Unicorn emulation, no Docker)")

    def _save(self):
        """Persist provisioning session to disk."""
        try:
            self._ani.save_all(str(self._SESSION_FILE))
        except Exception:
            pass
    ...
    @staticmethod
    def reset():
        """Delete cached session — triggers fresh provisioning on next run."""
        target = LinuxAnisetteProvider._SESSION_DIR
        if target.exists():
...
class AppleAuth:
    ...
    def _gsa_request(self, params: dict, timeout: int = 30) -> dict:
        ...
        # Use anisette-provided Client-Info (set during generate_cpd → generate_headers)
        client_info = getattr(self.anisette, '_last_client_info', CLIENT_INFO)

        # Debug: show what we're sending
        if os.environ.get("DEBUG_GSA"):
            cpd = body["Request"]["cpd"]
            print(f"[DEBUG] GSA op={params.get('o', '?')}")
            print(f"[DEBUG]   X-MMe-Client-Info (HTTP header): {client_info}")
            print(f"[DEBUG]   CPD X-Mme-Device-Id: {cpd.get('X-Mme-Device-Id', 'MISSING')}")
            print(f"[DEBUG]   CPD X-Mme-Client-Info: {cpd.get('X-Mme-Client-Info', 'MISSING')}")
            print(f"[DEBUG]   CPD X-Apple-I-MD: {cpd.get('X-Apple-I-MD', 'MISSING')[:40]}...")
            print(f"[DEBUG]   CPD X-Apple-I-MD-M: {cpd.get('X-Apple-I-MD-M', 'MISSING')[:40]}...")
            print(f"[DEBUG]   CPD X-Apple-I-MD-LU: {cpd.get('X-Apple-I-MD-LU', 'MISSING')}")
        ...
        if os.environ.get("DEBUG_GSA"):
            print(f"[DEBUG] GSA HTTP {resp.status_code}")
            print(f"[DEBUG] GSA response: {resp.content[:1000].decode('utf-8', errors='replace')}")
    ...
    def get_xcode_token(self) -> str:
        ...
        if os.environ.get("DEBUG_GSA"):
            print(f"[*] Xcode token type: {type(token).__name__}, length: {len(token)}")
...
class AppleSigner:
    ...
    def _sign_with_codesign(self, ipa_path: str, output_path: str,
        ...
        try:
            # 1. Create .p12 from PEM key + DER cert
            ...
            # Try -legacy first (modern OpenSSL 3.x), fall back without
            try:
                _sp.run([
                ...
                try:
                    ...
                    print(f"[!] openssl pkcs12 export failed: {_e2}")
                    return False

            if not Path(p12_enc_path).exists():
                print("[!] Failed to create .p12 file")
                return False

            # 1b. Create temp keychain and import p12
    ...
    def sign_ipa(self, ipa_path: str, output_path: str = None,
        ...
        try:
            ...
            if zsign_bin:
                ...
                final_profile = str(ws / f"{app_stem}.mobileprovision")
                final_ipa = str(ws / f"{app_stem}_patched_bundleid.ipa")
...
def interactive_login(anisette_url: str = DEFAULT_ANISETTE_URL, apple_id: str = None, password: str = None) -> AppleSigner:
    ...
    if NativeAnisetteProvider.is_available():
        ...
        if not AnisetteProvider.check_server(anisette_url):
            ...
            print()
            print("  Install the 'anisette' Python package (pip install anisette)")
            print("  or start an external anisette HTTP server on the URL above.")
            print()
            return None
